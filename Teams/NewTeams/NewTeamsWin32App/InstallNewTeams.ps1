########################################################################
# Installation Script for New Teams
...
Write-LogEntry -Value "Starting New Teams install process" -Severity 1
        ...
        try {
            ...
            if (Invoke-FileCertVerification -FilePath $SetupFilePath){
                ...
                Try {
                    ...
                    $Install.WaitForExit()
                    Write-LogEntry -Value "Finished New Teams Install" -Severity 1
                }
                catch [System.Exception] {
                    Write-LogEntry -Value  "Error running the New Teams install. Error Message: $($_.Exception.Message)" -Severity 3
                }
            }
            else {
...
Write-LogEntry -Value "Finished New Teams install process" -Severity 1
# Complete