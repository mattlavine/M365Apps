########################################################################
# Uninstall Script for New Teams via Teams Bootstrapper
...
Write-LogEntry -Value "Starting New Teams uninstall process" -Severity 1
        ...
        try {
            ...
            if (Invoke-FileCertVerification -FilePath $SetupFilePath){
                ...
                Try {
                    ...
                    Write-LogEntry -Value "Starting New Teams Uninstall with Win32App method" -Severity 1
                    $Install = Start-Process $SetupFilePath -ArgumentList "-x" -WindowStyle Hidden -PassThru -ErrorAction Stop
                    $Install.WaitForExit()
                    Write-LogEntry -Value "Finished New Teams Uninstall" -Severity 1
                }
                catch [System.Exception] {
                    Write-LogEntry -Value  "Error running the New Teams uninstall. Error Message: $($_.Exception.Message)" -Severity 3
                }
            }
            else {
...
Write-LogEntry -Value "Finished New Teams uninstall process" -Severity 1
# Complete